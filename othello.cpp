...
/*
  enum to represent player type
*/
typedef enum { HUMAN, COMPUTER } PlayerType;

/*
  struct to represent a Player Type
*/
typedef struct {
    PlayerType type;
    int lookahead; // Used only for COMPUTER
} Player;
...
/*
 * Basic board evaluation function to score a board state.
 */
int EvaluateBoard(Board b, int color) {
  int my_disks = CountBitsOnBoard(&b, color);
  int opponent_disks = CountBitsOnBoard(&b, OTHERCOLOR(color));
  return my_disks - opponent_disks;
}
...
int SequentialNegamax(Board board, int depth, Board legal_moves, int color) {
    int best_value = -64;
    for (int row = 1; row <= 8; ++row) {
        for (int col = 1; col <= 8; ++col) {
            if (legal_moves.disks[color] & BOARD_BIT(row, col)) {
                Board new_board = board; // Make a copy of the board
                Move m = {row, col};
                if (FlipDisks(m, &new_board, color, 0, 1) > 0) {
...
int ParallelNegamax(Board board, int depth, Board legal_moves, int color) {
    int best_value = -64;
    // Use a parallel loop to evaluate all legal moves
    // int grainsize = 64/(5 * __cilk_rts_getnworkers());
    ...
    cilk_for (int row = 1; row <= 8; ++row) {
        cilk_for (int col = 1; col <= 8; ++col) {
            if (legal_moves.disks[color] & BOARD_BIT(row, col)) {
                Board new_board = board; // Copy board to apply the move
                Move m = {row, col};
                FlipDisks(m, &new_board, color, 0, 1); // Apply the move
                int eval = -Negamax(new_board, depth - 1, OTHERCOLOR(color));
...
int Negamax(Board board, int depth, int color) {
  ...
  if (num_moves == 0 && EnumerateLegalMoves(board, OTHERCOLOR(color), &legal_moves) == 0) {
        return EvaluateBoard(board, color);
     } 
    else if (num_moves == 0) {
...
Move FindBestMove(Board board, int color, int depth) {
    ...
    EnumerateLegalMoves(board, color, &legal_moves);
    
    Move best_move = {-1, -1};
    int best_value = -64;

    // Iterate over all board positions to find the best move
    for (int row = 1; row <= 8; ++row) {
        for (int col = 1; col <= 8; ++col) {
            if (legal_moves.disks[color] & BOARD_BIT(row, col)) {
                Board new_board = board; // Copy board to apply the move
                Move m = {row, col};
                if (FlipDisks(m, &new_board, color, 0, 1) > 0) {
                  int move_val = -Negamax(new_board, depth - 1, OTHERCOLOR(color));
...
int PlayerTurn(Board *b, int color) {
  int move_made = 0;
  if ((color == X_BLACK && x_player.type == COMPUTER) || 
      (color == O_WHITE && o_player.type == COMPUTER)) {

    Move best_move = FindBestMove(*b, color, color == X_BLACK ? x_player.lookahead : o_player.lookahead);
    
    if (best_move.row != -1 && best_move.col != -1) {
      // Apply the bestMove
      FlipDisks(best_move, b, color, 1, 1); // Perform the move and flip opponent disks
      PlaceOrFlip(best_move, b, color); // Place the disk of the current color
      printf("Computer placed %c at %d,%d\n", diskcolor[color + 1], best_move.row, best_move.col);
      PrintBoard(*b);
      move_made = 1; // Indicate that a move was made
    } else {
      printf("No valid moves for Computer (%c).\n", diskcolor[color + 1]);
    }
  } else {
    move_made = HumanTurn(b, color);
  }
  return move_made;
}
...
void ChoosePlayerType() {
  char player_type;
  printf("Choose player for X (H for Human, C for Computer): ");
  scanf(" %c", &player_type);
  if (player_type == 'C' || player_type == 'c') {
      x_player.type = COMPUTER;
      printf("Enter lookahead depth for X: ");
      scanf("%d", &x_player.lookahead);
  } else {
      x_player.type = HUMAN;
  }
  printf("Choose player for O (H for Human, C for Computer): ");
  scanf(" %c", &player_type);
  if (player_type == 'C' || player_type == 'c') {
      o_player.type = COMPUTER;
      printf("Enter lookahead depth for O: ");
      scanf("%d", &o_player.lookahead);
  } else {
      o_player.type = HUMAN;
  }
}
...
int main (int argc, const char * argv[]) 
{
  ...
  do {
    move_possible =
      PlayerTurn(&gameboard, X_BLACK) | 
      PlayerTurn(&gameboard, O_WHITE);
  } while(move_possible);
  ...
  // Compute the duration in seconds with 3 decimal places
  std::chrono::duration<double> duration = end_time - start_time;
  printf("Game duration: %.3f seconds\n", duration.count());
